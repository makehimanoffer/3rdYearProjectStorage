...
bool Application::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_Terrain->Initialize(m_Direct3D->GetDevice(), "../hm.bmp", "../cm.bmp", 20.0f, L"../trippyTerrain.dds", 
				       L"../normal.dds");
	if(!result)
	{
	...
	result = m_SkyPlane->Initialize(m_Direct3D->GetDevice(), L"../tankClouds.dds", L"../binaryterrain.dds");
	if(!result)
	{
