...
bool Terrain::Initialize(ID3D11Device* device, char* heightMapFilename, float maximumHeight)
{
	bool result;
...
//In the BuildModel function we now create a second set of texture coordinates for 
//sampling the texture layer masks since they cover the entire 32x32 section and not just 
//the individual quads. So in the loop that builds the model we need to divide the number of quads 
//(31 quads since it is made of 32 height points) by one and use that as an increment value as we move along 
//the width and height of the terrain section.
...
void Terrain::CalculateTangentBinormal(TempVertexType vertex1, TempVertexType vertex2, TempVertexType vertex3, VectorType& tangent, 
					    VectorType& binormal)
{
