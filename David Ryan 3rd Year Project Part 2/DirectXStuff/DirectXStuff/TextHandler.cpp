...
bool TextHandler::InitializeSentence(Sentence** sentence, int maxLength, ID3D11Device* device)
{
	Vertex* vertices;
	...
	*sentence = new Sentence;
    ...
    vertexBuffDesc.ByteWidth = sizeof(Vertex) * (*sentence)->vertexCount;
    ...
    result = device->CreateBuffer(&vertexBuffDesc, &vertexData, &(*sentence)->vertexBuffer);
	if(FAILED(result))
	{
		return false;
	}
    ...
    indexBuffDesc.ByteWidth = sizeof(unsigned long) * (*sentence)->indexCount;
	...
	// Create index buffer.
	result = device->CreateBuffer(&indexBuffDesc, &indexData, &(*sentence)->indexBuffer);
	if(FAILED(result))
	{
		return false;
	}
...
bool TextHandler::UpdateSentence(Sentence* sentence, char* text, int positionX, int positionY, float red, float green, float blue,
...
void TextHandler::ReleaseSentence(Sentence** sentence)
{
	if(*sentence)
	{
...
bool TextHandler::RenderSentence(Sentence* sentence, ID3D11DeviceContext* deviceContext, FontShader* FontShader, D3DXMATRIX worldMx, 
							   D3DXMATRIX orthoMx)
{
