...
bool Input::Init(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	// Set data format.  In case since it is a keyboard can use predefined data format.
	...
	//ok. if set to DISCL_EXCLUSIVE only application has access to it.
	...
	// Initialize direct input interface for mouse.
...
//1. If focus is lost.
//2. If it becomes un-acquired. if happens. call acquire each frame
//
...
//next comes is escape pressed. can be used as a prototype for all types of button press
...
bool Input::IsPgUpPressed()
{
	...
	if(_keyboardState[DIK_SPACE] & 0x80)
	{
		return true;
	}
...
bool Input::IsPgDownPressed()
{
	...
	if(_keyboardState[DIK_BACKSPACE] & 0x80)
	{
		return true;
	}
