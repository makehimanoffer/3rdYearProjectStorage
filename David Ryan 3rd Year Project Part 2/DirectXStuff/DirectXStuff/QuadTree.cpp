...
bool QuadTree::Init(Terrain* terrain, ID3D11Device* device)
{
	int vertexCount;
	float centerX, centerZ, width;

	//first need to get all info from terrain class. get number of vertices  in terrain. use val to creat vert list.
	//sent to terrain class and filled with terra info
	...
	//then kill list once not needed
...
//calls render node. which goes through tree and renders nodes using frustum object which determines node visibility.
//must init drawcount to 0;
...
//return previous number of drawn in render
int QuadTree::GetDrawCount()
{
	return m_drawCount;
}

// calc mesh dimensions. goes through all vertices. uses them to calculate center of terrain and max width
...
//build tree . recursive. start with parent node. then call for child node and for each child . builds entire tree.
...
//render node is responsible for drawing visible nodes in quad tree. takes in frustum object and shader 
...
void QuadTree::RenderNode(NodeType* node, Frustum* frustum, ID3D11DeviceContext* deviceContext, TerrainShader* shader)
{
	...
	//start by doing a frustum check
