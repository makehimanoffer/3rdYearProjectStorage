...
bool Direct3DHandler::Init(int scrWidth, int scrHeight, bool vsync, HWND hwnd, bool fullscreen, 
						  float screenDepth, float screenNear)
{
	...
	{
		if(displayModeList[i].Width == (unsigned int)scrWidth)
		{
			if(displayModeList[i].Height == (unsigned int)scrHeight)
			{
    ...
    _viewport.Width = (float)scrWidth;
    _viewport.Height = (float)scrHeight;
	...
	screenAspect = (float)scrWidth / (float)scrHeight;
	...
	D3DXMatrixOrthoLH(&_orthoMx, (float)scrWidth, (float)scrHeight, screenNear, screenDepth);
...
void Direct3DHandler::GetProjectionMatrix(D3DXMATRIX& projectionMx)
{
...
void Direct3DHandler::GetWorldMatrix(D3DXMATRIX& worldMx)
{
	worldMx = _worldMx;
	return;
}
...
void Direct3DHandler::GetOrthoMatrix(D3DXMATRIX& orthoMx)
{
	orthoMx = _orthoMx;
	return;
}
