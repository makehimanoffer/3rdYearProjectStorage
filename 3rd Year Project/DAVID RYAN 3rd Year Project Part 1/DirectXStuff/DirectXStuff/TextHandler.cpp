						   ...
						   D3DXMATRIX baseViewMx)
{
	...
	if(!result)
	{
		MessageBox(hwnd, L"Could not Init font object.", L"Error", MB_OK);
		return false;
	}

	// Init first sentence.
	result = InitSentence(&_sentence1, 150, device);
	if(!result)
	{
		return false;
	}
	...
	result = InitSentence(&_sentence2, 32, device);
	if(!result)
	{
		return false;
	}
	...
	// Init fourth sentence.
	result = InitSentence(&_sentence4, 16, device);
	if(!result)
	{
		return false;
	}
	...
	// Init fifth sentence.
	result = InitSentence(&_sentence5, 16, device);
	if(!result)
	{
		return false;
	}
	...
	// Init sixth sentence.
	result = InitSentence(&_sentence6, 16, device);
	if(!result)
	{
		return false;
	}

	// Init seventh sentence.
	result = InitSentence(&_sentence7, 16, device);
	if(!result)
	{
		return false;
	}

	// Init eighth sentence.
	result = InitSentence(&_sentence8, 16, device);
	if(!result)
	{
		return false;
	}
	...
	// Init ninth sentence.
	result = InitSentence(&_sentence9, 16, device);
	if(!result)
	{
		return false;
	}
	...
	// Init tenth sentence.
	result = InitSentence(&_sentence10, 16, device);
	if(!result)
	{
		return false;
	}

	// Init eleventh sentence.
	result = InitSentence(&_sentence11, 32, device);
	if(!result)
	{
		return false;
	}
							   ...
							   D3DXMATRIX orthoMx)
{
	...
	pixelColor = D3DXVECTOR4(sentence->red, sentence->blue, sentence->blue, 1.0f);
