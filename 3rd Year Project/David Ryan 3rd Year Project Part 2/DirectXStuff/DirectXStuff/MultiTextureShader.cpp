...
bool MultiTextureShader::Init(ID3D11Device* device, HWND hwnd)
{
	bool result;

	//the multitexture shaders are loaded below.
...
void MultiTextureShader::KillSafely()
{
...
//the render takes a pointer to texture array as input.
//gives shader access to the two texture resources.
...
//init shader loads vertex and pixel shaders 
//also sets up the layout, matrix buffer and sample state
bool MultiTextureShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	//multitexture vertex shader loaded here:
	...
	//The PIXEL SHADER IS LOADED HERE:
...
//THE SHUTDOWN SHADER FUNCTION RELEASES all interfaces setup in the initialize shader function
...
//OutputShaderErrorMessage outputs an error to a file  if there a compile error with the shader files.
...
//set Shader Parameters sets the matrices and texture arrays  in shader before rendering
						  ...
						  ID3D11ShaderResourceView** textureArray)
{
	...
	//below is where the texture array is being set before rendering 
	// Set shader texture array resource in the pixel shader.
	//the PSSetShaderResources is used to set the texture array.
	//the first param is where to start in array.
	//the second is how many texture there are in the array being passed in.
	//the third is a pointer to the actual array.
...
//this sets the layout, shaders and the sampler. then draws using shader. 
