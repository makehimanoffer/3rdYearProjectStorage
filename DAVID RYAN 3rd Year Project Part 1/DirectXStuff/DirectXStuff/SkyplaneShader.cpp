...
bool SkyplaneShader::Init(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitShader(device, hwnd, L"../skyplane.vs", L"../skyplane.ps");
	if(!result)
	{
		return false;
	}
...
bool SkyplaneShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
...
bool SkyplaneShader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
...
bool SkyplaneShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, 
