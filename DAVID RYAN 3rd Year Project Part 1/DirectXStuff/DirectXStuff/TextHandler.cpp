...
TextHandler::TextHandler()
{
	...
	m_sentence9 = 0;
	m_sentence10 = 0;
	m_sentence11 = 0; //T 0.7
}


TextHandler::TextHandler(const TextHandler& other)
{
}
...
bool TextHandler::Init(ID3D11Device* device, ID3D11DeviceContext* deviceContext, HWND hwnd, int screenWidth, int screenHeight, 
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = InitSentence(&m_sentence1, 150, device);
	if(!result)
	{
		return false;
	}
	...
	result = InitSentence(&m_sentence2, 32, device);
	if(!result)
	{
		return false;
	}

	// Init the third sentence.
	result = InitSentence(&m_sentence3, 16, device);
	if(!result)
	{
		return false;
	}

	// Init the fourth sentence.
	result = InitSentence(&m_sentence4, 16, device);
	if(!result)
	{
		return false;
	}

	// Init the fifth sentence.
	result = InitSentence(&m_sentence5, 16, device);
	if(!result)
	{
		return false;
	}

	// Init the sixth sentence.
	result = InitSentence(&m_sentence6, 16, device);
	if(!result)
	{
		return false;
	}

	// Init the seventh sentence.
	result = InitSentence(&m_sentence7, 16, device);
	if(!result)
	{
		return false;
	}

	// Init the eighth sentence.
	result = InitSentence(&m_sentence8, 16, device);
	if(!result)
	{
		return false;
	}

	// Init the ninth sentence.
	result = InitSentence(&m_sentence9, 16, device);
	if(!result)
	{
		return false;
	}

	// Init the tenth sentence.
	result = InitSentence(&m_sentence10, 16, device);
	if(!result)
	{
		return false;
	}

	// Init the eleventh sentence.
	result = InitSentence(&m_sentence11, 32, device);
	if(!result)
	{
		return false;
	}
...
void TextHandler::KillSafely()
{
	...
	ReleaseSentence(&m_sentence11); //T 0.7

	return;
}


bool TextHandler::Render(ID3D11DeviceContext* deviceContext, FontShader* FontShader, D3DXMATRIX worldMatrix, D3DXMATRIX orthoMatrix)
{
	bool result;
...
bool TextHandler::InitSentence(SentenceType** sentence, int maxLength, ID3D11Device* device)
{
	VertexType* vertices;
	...
	*sentence = new SentenceType;
	...
	// Init vertex array to zeros at first.
...
bool TextHandler::RenderSentence(SentenceType* sentence, ID3D11DeviceContext* deviceContext, FontShader* FontShader, D3DXMATRIX worldMatrix, 
