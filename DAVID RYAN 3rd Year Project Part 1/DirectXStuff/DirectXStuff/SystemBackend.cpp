...
//MASSIVE CHANGES MADE TO THIS CLASS in favour of an encapsulated format in the application class
...
SystemBackend::SystemBackend(const SystemBackend& other)
{
}
...
bool SystemBackend::Init()
{
	...
	// Init the application wrapper object.
	result = m_Application->Init(m_hinstance, m_hwnd, screenWidth, screenHeight);
	if(!result)
	{
		return false;
	}
...
void SystemBackend::KillSafely()
{
	...
	if(m_Application)
	{
		m_Application->KillSafely();
		delete m_Application;
		m_Application = 0;
	}

	// KillSafely the window.
	KillSafelyWindows();
...
void SystemBackend::InitWindows(int& screenWidth, int& screenHeight)
{
	WNDCLASSEX wc;
	...
	m_applicationName = L"David Ryan 3rd year Project";
