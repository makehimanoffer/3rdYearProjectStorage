...
void Camera::GetViewMx(D3DXMATRIX& viewMx)
{
	viewMx = _viewMx;
	return;
}
...
void Camera::GenerateBaseViewMx()
{
	D3DXVECTOR3 up, position, lookAt;
	float yaw, pitch, roll;
	D3DXMATRIX rotationMx;
...
void Camera::GetBaseViewMx(D3DXMATRIX& viewMx)
{
	viewMx = _baseViewMx;
	return;
}
...
void Camera::RenderReflection(float height)
{
	...
	// Setup position of camera in world.  For planar reflection invert Y position of camera.
	position.x = _posX;
	position.y = -_posY + (height * 2.0f);
	position.z = _posZ;
	...
	// Set yaw (Y axis), pitch (X axis), and roll (Z axis) rotations in radians.  Invert X rotation for reflection.
	pitch = -_rotX * 0.0174532925f;
	yaw   = _rotY * 0.0174532925f;
	roll  = _rotZ * 0.0174532925f;
	...
	// Finally create reflection view matrix from three updated vectors.
	D3DXMatrixLookAtLH(&_reflectionViewMx, &position, &lookAt, &up);

	return;
}
//There is also a new function to retrieve reflection view matrix.

void Camera::GetReflectionViewMx(D3DXMATRIX& viewMx)
{
	viewMx = _reflectionViewMx;
	return;
}