...
bool SkyDomeShader::Init(ID3D11Device* device, HWND hwnd)
{
	bool result;


	// Init the vertex and pixel shaders.
	result = InitShader(device, hwnd, L"../skydome.vs", L"../skydome.ps");
	if(!result)
	{
		return false;
	}
...
bool SkyDomeShader::InitShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	HRESULT result;
