							  ...
							  WCHAR* normalTextureFilename)
{
	...
	// Init the vertex and index buffer that hold the geometry for the terrain.
...
void Terrain::CalculateModelVectors()
{
	...
	// Init the index to the model data.
	index = 0;
...
bool Terrain::InitBuffers(ID3D11Device* device)
{
	...
	for(i=0; i<m_vertexCount; i++)
	{
		...
		vertices[i].color    = D3DXVECTOR4(m_model[i].r, m_model[i].g, m_model[i].b, 1.0f);
...
bool Terrain::LoadTextures(ID3D11Device* device, WCHAR* colorTexturefilename, WCHAR* normalTextureFilename)
{
	...
	// Init the normal map texture object.
	result = m_NormalTexture->Init(device, normalTextureFilename);
	if(!result)
	{
		return false;
	}
