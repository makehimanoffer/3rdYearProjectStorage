...
bool Application::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_Model->Initialize(m_Direct3D->GetDevice(), "../MetalGearRex.txt", L"../tankTexture.dds");
	if(!result)
	{
	...
	result = m_Hud->Initialize(m_Direct3D->GetDevice(), screenWidth, screenHeight, L"../hud1.dds", 800, 600);
	if(!result)
	{
	...
	result = m_Terrain->Initialize(m_Direct3D->GetDevice(), "../test1.bmp", L"../battlefield.dds", L"../bump.dds");
	if(!result)
	{
