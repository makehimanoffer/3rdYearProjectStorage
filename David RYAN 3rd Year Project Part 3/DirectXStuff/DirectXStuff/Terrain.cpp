...
bool Terrain::Init(ID3D11Device* device, char* heightMapFilename, WCHAR* colorTextureFilename, WCHAR* normalMapFilename)
{
	...
	/*The flow of the Initialize function has changed a bit. We load the height map as usual and then reduce its height. 
...
bool Terrain::LoadHeightMap(char* filename)
{
	...
	for(j=0; j<m_terrainHeight; j++)
	{
		for(i=0; i<m_terrainWidth; i++)
		{
			...
			m_heightMap[index].x = (float)i*100;
			m_heightMap[index].y = (float)height;
			m_heightMap[index].z = (float)j*100;
...
void Terrain::ReduceHeightMap()
{
	...
	for(j=0; j<m_terrainHeight; j++)
	{
		for(i=0; i<m_terrainWidth; i++)
		{
			m_heightMap[(m_terrainWidth * j) + i].y /=20.0f;
		}
	}
...
bool Terrain::LoadTextures(ID3D11Device* device, WCHAR* colorTexturefilename, WCHAR* normalMapFilename)
{
	...
	result = m_ColorTexture->Init(device, colorTexturefilename);
	if(!result)
	{
		return false;
	}
	...
	m_NormalMapTexture = new Texture;
	...
	result = m_NormalMapTexture->Init(device, normalMapFilename);
	if(!result)
	{
		return false;
	}
