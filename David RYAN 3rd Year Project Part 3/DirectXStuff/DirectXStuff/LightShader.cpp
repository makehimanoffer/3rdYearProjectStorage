...
bool LightShader::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMx, D3DXMATRIX viewMx, 
			      D3DXMATRIX projectionMx, ID3D11ShaderResourceView* texture, D3DXVECTOR3 lightDirection, D3DXVECTOR4 ambientColor,
			      D3DXVECTOR4 diffuseColor)
{
	...
	result = SetShaderParameters(deviceContext, worldMx, viewMx, projectionMx, texture, lightDirection, ambientColor, diffuseColor);
	if(!result)
	{
		return false;
	}
...
bool LightShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// This setup needs to match VertexType stucture in ModelClass and in shader.
...
bool LightShader::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMx, D3DXMATRIX viewMx, 
					   D3DXMATRIX projectionMx, ID3D11ShaderResourceView* texture, D3DXVECTOR3 lightDirection, 
					   D3DXVECTOR4 ambientColor, D3DXVECTOR4 diffuseColor)
{
	...
	pointerToData2->ambientColor = ambientColor;
