...
bool ColorShader::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	// This setup needs to match VertexType stucture in Model and in shader.
					   ...
					   D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix)
{
	...
	MatrixBufferType* pointerToData;
	...
	pointerToData = (MatrixBufferType*)mappedResource.pData;

	// Copy matrices into constant buffer.
	pointerToData->world = worldMatrix;
	pointerToData->view = viewMatrix;
	pointerToData->projection = projectionMatrix;
