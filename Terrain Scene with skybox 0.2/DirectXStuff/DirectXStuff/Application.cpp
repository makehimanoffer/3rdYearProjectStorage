...
bool Application::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_ColorTexture1->Initialize(m_Direct3D->GetDevice(), L"../dirt001.dds");
	if(!result)
	{
		return false;
	}
	...
	result = m_ColorTexture2->Initialize(m_Direct3D->GetDevice(), L"../dirt004.dds");
	if(!result)
	{
		return false;
	}
	...
	result = m_ColorTexture3->Initialize(m_Direct3D->GetDevice(), L"../dirt002.dds");
	if(!result)
	{
		return false;
	}
	...
	result = m_ColorTexture4->Initialize(m_Direct3D->GetDevice(), L"../stone001.dds");
	if(!result)
	{
		return false;
	}
	...
	result = m_AlphaTexture1->Initialize(m_Direct3D->GetDevice(), L"../alpha001.dds");
	if(!result)
	{
		return false;
	}
	...
	// Load the first alpha texture object.
	result = m_NormalTexture1->Initialize(m_Direct3D->GetDevice(), L"../normal001.dds");
	if(!result)
	{
		return false;
	}
	...
	result = m_NormalTexture2->Initialize(m_Direct3D->GetDevice(), L"../normal002.dds");
	if(!result)
	{
		return false;
	}
	...
	result = m_SkyPlane->Initialize(m_Direct3D->GetDevice(), L"../cloud001.dds", L"../perturb001.dds");
	if(!result)
	{
	...
	result = m_Water->Initialize(m_Direct3D->GetDevice(), L"../waternormal.dds", 3.75f, 110.0f);
	if(!result)
	{
