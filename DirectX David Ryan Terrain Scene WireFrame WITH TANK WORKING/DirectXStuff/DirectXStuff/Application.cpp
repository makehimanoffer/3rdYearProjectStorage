...
Application::Application()
{
	...
	m_TerrainShader = 0; //NEW IN TERRAIN 0.3
	m_Light = 0; //NEW IN TERRAIN 0.3

	m_Frustum = 0; // NEW IN TERRAIN 0.7
	m_QuadTree = 0; // NEW IN TERRAIN 0.7
...
bool Application::Initialize(HINSTANCE hinstance, HWND hwnd, int screenWidth, int screenHeight)
{
	...
	result = m_Terrain->Initialize(m_Direct3D->GetDevice(), "../heightmap01.bmp", L"../binaryterrain.dds");
	if(!result)
	{
	...
	// Initialize the terrain object. NEW AND UPDATED IN TERR 0.6
	result = m_Model->Initialize(m_Direct3D->GetDevice(), "../T_302.txt", L"../trippyTerrain.dds");
	if(!result)
	{
	...
	// Create the quad tree object. T 0.7
	...
	// Initialize the quad tree object. T 0.7
...
void Application::Shutdown()
{
	...
	// Release the quad tree object. T 0.7
	if(m_QuadTree)
	{
		m_QuadTree->Shutdown();
		delete m_QuadTree;
	...
	// Release the frustum object. T 0.7
	if(m_Frustum)
	{
		delete m_Frustum;
		m_Frustum = 0;
