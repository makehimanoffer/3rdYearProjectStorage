						   ...
						   D3DXMATRIX baseViewMatrix)
{
	...
	result = UpdateSentence(m_sentence1, "Hello", 30, 30, 1.0f, 1.0f, 1.0f, deviceContext);
	if(!result)
	{
		return false;
	}
...
//UpdateSentence does what said on the tin. Uses map and unmap functions along with memcpy to update contents of vert buffer
			       ...
			       ID3D11DeviceContext* deviceContext)
{
	...
	//calc starting sentence to draw sentence

	// Calculate the X and Y pixel position on the screen to start drawing to.
	drawX = (float)(((m_screenWidth / 2) * -1) + positionX);
	drawY = (float)((m_screenHeight / 2) - positionY);

	//build vert array using fontclass and sentence info
	// Use the font class to build the vertex array from the sentence text and sentence draw location.
	m_Font->BuildVertexArray((void*)vertices, text, drawX, drawY);

	//copy vert  array info into sentence buffer
...
//The RenderSentence function puts the sentence vertex and index buffer on the input assembler and then 
//calls the FontShaderClass object to draw the sentence that was given as input to this function. 
...
//THIS IS NEW IN 0.13.
// Deals with outputting mouse positions to text.
...
//SetCPU converts it to a string. then stored in the sentence structure and rendered
bool TextClass::SetCpu(int cpu, ID3D11DeviceContext* deviceContext)
{
	...
	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence2, cpuString, 50, 20, 0.0f, 1.0f, 0.0f, deviceContext);
	if(!result)
	{
		return false;
	}
