...
void TerrainClass::NormalizeHeightMap()
{
	...
	for(j=0; j<m_terrainHeight; j++)
	{
		for(i=0; i<m_terrainWidth; i++)
		{
			m_heightMap[(m_terrainHeight * j) + i].y /= 40.0f;
		}
	}
...
// in TERRAIN 0.7 init buffers no longer builds vert and index buffer/array . only build vert array which is now private so named m_vertices instead of vertices
...
{
	// now only Releases the vertex array.
	if(m_vertices)
	{
		delete [] m_vertices;
		m_vertices = 0;
